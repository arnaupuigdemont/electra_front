...
const ReusableConfirmModal: React.FC<ReusableConfirmModalProps> = ({
  isOpen,
  title,
  message,
  confirmText = 'Confirm',
  ...
  return (
    <div style={styles.backdrop}>
      <div style={styles.modal} role="dialog" aria-modal="true" aria-labelledby="confirm-title">
        {title && <div id="confirm-title" style={styles.title}>{title}</div>}
        <div style={styles.message}>{message}</div>
        {errorText && <div style={styles.error}>{errorText}</div>}
        <div style={styles.actions}>
          <button
            style={{ ...styles.button, ...(confirmColor === 'danger' ? styles.danger : styles.primary) }}
            onClick={onConfirm}
            disabled={isBusy}
          >
